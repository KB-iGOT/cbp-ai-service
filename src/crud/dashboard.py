...
class CRUDDashboard:
    """
    CRUD methods for the Dashboard.
    """
    ...
    async def fetch_dashboard_metrics(
        ...
        - user_id=None  → super-admin view (org-level filters, returns all fields)
        - user_id set   → public-user view (scoped to that user, subset of fields)
        """
        is_admin = user_id is None
        try:
            # ── Role mapping aggregate query ──────────────────────────────────
            if is_admin:
                stmt = select(
            ...
            if is_admin:
                ...
                cbp_stmt = cbp_stmt.where(CBPPlan.user_id == user_id)
                if filters.date_range:
                    if filters.date_range.from_date:
                        cbp_stmt = cbp_stmt.where(func.date(CBPPlan.created_at) >= filters.date_range.from_date)
                    if filters.date_range.to_date:
            ...
            # ── Role mappings with recommendations ────────────────────────────
            ...
            if is_admin:
                ...
                rec_stmt = rec_stmt.where(RoleMapping.user_id == user_id)
                if filters.date_range:
                    if filters.date_range.from_date:
                        rec_stmt = rec_stmt.where(func.date(RoleMapping.created_at) >= filters.date_range.from_date)
                    if filters.date_range.to_date:
            ...
            # Counts from users table; ministry filter uses organization_ids ARRAY (&&).
            # Department filter not applied — organization_ids has no dept IDs.
            total_users = 0
            if is_admin:
            ...
            if is_admin:
                ...
                saved_rec_stmt = saved_rec_stmt.where(CBPPlan.user_id == user_id)
                if filters.date_range:
                    if filters.date_range.from_date:
                        saved_rec_stmt = saved_rec_stmt.where(func.date(CBPPlan.created_at) >= filters.date_range.from_date)
                    if filters.date_range.to_date:
            ...
            # ── Build response ────────────────────────────────────────────────
            if is_admin:
                print(f"[Dashboard Metrics][admin] total_users={total_users}, total_role_mappings={row.total_role_mappings if row else 0}")
                return {
                ...
                print(f"[Dashboard Metrics][user] user_id={user_id}, total_role_mappings={row.total_role_mappings if row else 0}")
                return {
                    "total_role_mappings": row.total_role_mappings if row else 0,
                    "unique_role_mappings": row.unique_role_mappings if row else 0,
                    "role_mappings_with_recommendations": role_mappings_with_recommendations,
            ...
            print(f"Database error while fetching dashboard metrics: {str(e)}")
            await db.rollback()
            raise DashboardQueryError("Database error while fetching dashboard metrics") from e
    ...
    async def fetch_gap_analysis(
        ...
        - user_id=None  → super-admin view (org-level filters)
        - user_id set   → public-user view (scoped to that user)
        SQL query logic is unchanged; only the WHERE clause differs.
        """
        try:
            ...
            if user_id is not None:
                # User-scoped: filter by user_id only
                where_clauses.append("rm.user_id = :user_id")
                params["user_id"] = str(user_id)
            else:
                # Admin: filter by ministry / department
                if filters.ministries and len(filters.ministries) > 0:
