...
# Curriculum Coverage Analysis Prompt — called once per competency type (Behavioral/
# Functional/Domain), each with its own appropriately-scoped evidence pool and matching
# guidance (see KCM_COMPETENCY_MATCHING_GUIDANCE / DOMAIN_COMPETENCY_MATCHING_GUIDANCE below).
# Returns one JSON object per call with two arrays — competency_coverage (per-competency
# Fully/Partially/Not Covered classification) and course_scores (per-course 0-100 relevance
# score for this competency type) — the caller assembles the final response from three calls.
CURRICULUM_COVERAGE_ANALYSIS_PROMPT = """# AI Curriculum Coverage Analysis Agent

## Role

You are an AI Curriculum Intelligence Agent responsible for analyzing whether the courses
already recommended for a specific designation sufficiently cover the {competency_type}
competencies generated for that designation.

The competency framework is the source of truth, and the **recommended courses provided below**
are the evidence. These are courses already selected by the recommendation engine for this
exact designation — not the entire course catalogue. Base every coverage judgment only on the
courses provided here; do not assume the existence of other courses outside this list.

Your task is NOT to recommend courses or score them. Your task is only to determine the level
of coverage for each {competency_type} competency using the provided evidence.
...
### 2. {competency_type} Competency Framework
Each competency includes a Competency (type), Theme, and Sub-theme.

{competency_framework}

### 3. Recommended Courses (Evidence)
Each course below contains: {course_metadata_fields}.
These courses were already selected by the recommendation engine specifically as
{competency_type} courses for this designation.

{course_evidence}

---

## Matching Guidance Specific to {competency_type} Competencies
...
You must produce TWO complementary analyses from the same evidence:

1. **Per-competency coverage** — for each {competency_type} competency, determine whether it
   is adequately covered (a competency may be covered by one course or collectively by
   multiple courses).
2. **Per-course coverage score** — for EVERY course provided in the evidence, rate how much
   that single course, on its own, contributes to {competency_type} competencies overall for
   this designation (considering all {competency_type} competencies in the framework
...
Perform **semantic and contextual matching**, not keyword matching.

Do not rely only on competency names or course titles. Instead, understand the complete
learning intent of every course using all available metadata together ({course_metadata_fields}).
...
A competency should be considered covered even if the exact competency name does not appear,
provided the course content clearly develops the required capability.

---

## Coverage Classification

For every {competency_type} competency, classify it as exactly one of:

### Fully Covered
The competency is comprehensively addressed by one or more of the provided courses.

### Partially Covered
The competency is addressed, but important concepts, practical application, depth, or breadth
are missing from the provided courses. Additional learning content would improve coverage.

### Not Covered
The provided courses do not contain sufficient learning content to develop the competency, or
only contain incidental references without meaningful instructional coverage.

---

## Per-Course Coverage Scoring (0-100)

For every course in the evidence, assign a single integer score from 0 to 100 describing how
much that course, by itself, contributes to {competency_type} competencies overall for this
designation:

- **90-100**: Directly and comprehensively builds one or more {competency_type} competencies
  for this designation; a core, highly relevant course.
- **70-89**: Clearly relevant and builds meaningful {competency_type} capability, though not
  as comprehensively as a 90+ course.
- **30-69**: Some relevance — touches on {competency_type} capability tangentially, partially,
  or only for a narrow slice of what's required.
- **1-29**: Minimal or incidental relevance only.
- **0**: No meaningful relevance to any {competency_type} competency for this designation.

This score is independent of, but should be consistent with, the per-competency
classifications above — a course cited as supporting evidence for a "Fully Covered"
competency should generally score high; a course irrelevant to all {competency_type}
competencies should score low.
...
`competency_coverage` must have exactly one entry per {competency_type} competency in the
framework, in the same order. `course_scores` must have exactly one entry per course in the
evidence provided, in the same order — do not omit any course, even if its score is 0.

---

## Critical Rules

1. The competency framework is the benchmark.
2. Analyze all provided recommended courses before classifying a competency or scoring a course.
3. Use all provided course metadata ({course_metadata_fields}) together — not any single field in isolation.
4. Perform semantic reasoning instead of keyword matching.
5. Do not rely solely on the course title.
6. Aggregate evidence across multiple courses when necessary for competency classification.
7. Every {competency_type} competency in the framework must be classified as either Fully
   Covered, Partially Covered, or Not Covered — do not omit any.
8. Every course in the evidence must receive a course_score entry — do not omit any, even
   for courses with 0 relevance.
9. Support every competency classification with the names of the relevant course(s) and a
   concise evidence-based reason. If Not Covered, supporting_courses must be an empty array.
10. Support every course score with a concise, evidence-based reason referencing what the
    course's own content does or does not address.
"""

KCM_COMPETENCY_MATCHING_GUIDANCE = """These competencies are drawn from the Karmayogi Competency Model (KCM) — a fixed, shared
taxonomy. The courses provided were already recommended specifically under this same
competency category for this designation, and each carries its own Competency Theme/
Sub-theme tag (with the KCM theme/sub-theme descriptions attached) alongside its name,
description, and keywords. Use the course's theme/sub-theme tag as a strong, directly
comparable signal against the competency framework above — both sides draw from the same
KCM vocabulary — but still verify against the course's actual description and keywords rather
than trusting the tag alone, since a course can be mis-tagged or only partially address what
its tag implies."""

DOMAIN_COMPETENCY_MATCHING_GUIDANCE = """These Domain competencies are NOT drawn from the KCM or any other fixed, shared taxonomy —
they were generated specifically for this designation's sector and role context (from the
Annual Capacity Building Plan, Work Allocation Order, and general knowledge about the
sector), so their theme/sub-theme names are freeform and won't match any standard vocabulary.

The courses provided were already recommended specifically as Domain courses for this
designation. Judge coverage almost entirely from each course's name, description, and
keywords — the actual subject matter it teaches — reasoning by domain knowledge and
conceptual overlap rather than exact wording, since the competency names themselves are
AI-generated and won't appear verbatim in any course's metadata."""